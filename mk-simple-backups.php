				...
				case "createBackup":
					...
					if(count($bkp_options) > 0) {
						...
						if($s == true) {
							...
							if(is_array($bkp->tmp_files)) if(@count($bkp->tmp_files > 0)) $bkp->removeFiles( $bkp->tmp_files );
			...
			if(count($msg) > 0) {
				...
				<div id="message" class="updated">
					...
					foreach($msg AS $m) {
						...
						if(isset($m["error"]) AND $m["error"] == true) $class = ' class="error"';
						else unset($class);
